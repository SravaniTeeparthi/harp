...
Description
-----------
Draws hand bounding boxes on a video in green. It also prints
confidence score on top of each box.
    ...
    while(cap.isOpened()):
        if np.sum(frame) !=None :
            ...
            for i in range(0,l):
                ...
                if result == None:
                    cv2.putText(frame,"Hands not found",(30,50), font,1,(0,0,255),3,cv2.LINE_AA)
            out.write(frame)
            #cv2.imshow("image", frame)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
            poc += skp_frms
            print("Writing frame:",poc)
